...
@section('content')
  ...
  <div class="content-wrapper">
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Thống kê nhà thầu tham dự gói thầu CNTT theo địa bàn</h1>
          </div>
          <div class="col-sm-6">
      ...
      contentTemplate(contentElement) {
        $('<div class="h-100" />').addClass('drill-down').dxDataGrid({
            ...
            columns: [{
              ...
              caption: 'Nhà thầu tham dự',
              dataField: 'name_vi_join',
              dataType: 'string',
              cellTemplate: function (container, options) {
                  if(options.value == null){
                    return;
                  }
                  $('<a>' + options.value + '</a>').attr('href', `{{url('/dashboard/detail-competitors')}}?id=${options.data.id_join}`).attr('target', '_blank').appendTo(container);
              }
            }, {
              caption: 'Trạng thái',
          ...
          if (e.area === 'data') {
            ...
            drillDownDataSource = new DevExpress.data.DataSource({
              ...
              load: function(loadOptions) {
                  ...
                  $.getJSON(`{{url('/api/statistic-competitor-list')}}?city=${rowPathName}&year=${year}&yearmonth=${yearmonth}`, params)
                      .done(function(result) {
                          console.log(result);
