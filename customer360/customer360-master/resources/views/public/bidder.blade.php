...
@section('script')
<script>
	var competitors = new DevExpress.data.CustomStore({
		key: "id",
		loadMode: "raw",
		...
		remove: function(key) {
            ...
            var deferred = $.Deferred();
            $.ajax({
                url: "{{url('/competitor/destroy')}}" +"/"+ encodeURIComponent(key),
                method: "DELETE",
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
				}
        ...
        update: function(key, values) {
            ...
            var deferred = $.Deferred();
            $.ajax({
                url: "{{url('/competitor/edit')}}" + "/" + encodeURIComponent(key),
                method: "PUT",
                data: JSON.stringify(values),
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
	...
	const treeList = $("#competitors").dxTreeList({
		...
		rowDragging: {
			...
			onReorder(e) {
				...
				if (e.dropInsideItem) {
					...
					competitors.update(e.itemData.id, { head_id: e.itemData.head_id }).then(() => {
					...
					const sourceIndex = competitors.indexOf(sourceData);
					competitors.splice(sourceIndex, 1);

					const targetIndex = competitors.indexOf(targetData) + 1;
					competitors.splice(targetIndex, 0, sourceData);
				}
