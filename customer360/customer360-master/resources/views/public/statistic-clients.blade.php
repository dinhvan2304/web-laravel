    ...
    const pivotGrid = $('#gridContainer').dxPivotGrid({
        ...
        onCellClick(e) {
          if (e.area === 'data') {
            ...
            var filterArr = e.component.getDataSource().store().getDrillDownItems().filter((element) => { return element.company_name == rowPathName; });
            var client_mst = filterArr[0].so_dkkd;

            var yearmonth = e.cell.columnPath[0];
            ...
            drillDownDataSource = new DevExpress.data.DataSource({
              ...
              load: function(loadOptions) {
                  ...
                  $.getJSON(`{{url('/api/statistic-clients-list')}}?client_mst=${client_mst}&yearmonth=${yearmonth}`, params)
                      .done(function(result) {
                          console.log(result);
