...
@section('content')
  ...
  <div class="content-wrapper">
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Thống kê danh sách gói thầu CNTT theo đối thủ</h1>
          </div>
          <div class="col-sm-6">
...
<script>
  ...
  $.getJSON("{{url('/api/statistic-packages')}}")
    ...
    const pivotGridChart = $('#pivotgrid-chart').dxChart({
      ...
      tooltip: {
        ...
        customizeTooltip(args) {
          const valueText = (args.seriesName.indexOf('Tổng') !== -1)
    ...
    const pivotGrid = $('#gridContainer').dxPivotGrid({
        ...
        onCellClick(e) {
          if (e.area === 'data') {
            ...
            var filterArr = e.component.getDataSource().store().getDrillDownItems().filter((element) => { return element.name_vi == rowPathName; });
            var competitor_mst = filterArr[0].so_dkkd;
            ...
            drillDownDataSource = new DevExpress.data.DataSource({
              ...
              load: function(loadOptions) {
                  ...
                  $.getJSON(`{{url('/api/statistic-packages-list')}}?competitor_mst=${competitor_mst}&year=${year}&yearmonth=${yearmonth}`, params)
                      .done(function(result) {
                          console.log(result);
        ...
        onCellPrepared(e) {
          const { cell } = e;
          cell.area = e.area;
          if (cell.area === 'data' && cell.rowType === 'D' && cell.columnType === 'D') {
            Object.assign(e.cellElement.get(0).style, {"cursor": "pointer", "color": `#007bff`});
          }
        },
