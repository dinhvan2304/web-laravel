...
@section('script')
<script>
	var provinces = new DevExpress.data.CustomStore({
		key: "id",
		loadMode: "raw",
		...
		remove: function(key) {
            ...
            var deferred = $.Deferred();
            $.ajax({
                url: "{{url('/provinces/destroy')}}" +"/"+ encodeURIComponent(key),
                method: "DELETE",
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
				}
        ...
        update: function(key, values) {
            ...
            var deferred = $.Deferred();
            $.ajax({
                url: "{{url('/provinces/edit')}}" + "/" + encodeURIComponent(key),
                method: "PUT",
                data: JSON.stringify(values),
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
	...
	const treeList = $("#provinces").dxTreeList({
		...
		rowDragging: {
			...
			onReorder(e) {
				...
				if (e.dropInsideItem) {
					...
					const sourceIndex = provinces.indexOf(sourceData);
					provinces.splice(sourceIndex, 1);

					const targetIndex = provinces.indexOf(targetData) + 1;
					provinces.splice(targetIndex, 0, sourceData);
				}
